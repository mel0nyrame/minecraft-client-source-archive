var scriptName = "NewAAC5criticals";
...
function CriticalsFixed() {
    var mode = value.createList("Mode", [ "HytPacket",  "AntiCheat"], "Packet")
    ...
    this.onAttack = function(event) {
        ...
        if(target != null && mc.thePlayer.onGround) {
            switch(mode.get()) {
                ...
                case "HytPacket":
                    if(timer.hasTimePassed(delay.get()) && target.hurtTime <= hurttime.get()) {
                        mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(mc.thePlayer.posX, mc.thePlayer.posY + 0.009, mc.thePlayer.posZ, false))
                        mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(mc.thePlayer.posX, mc.thePlayer.posY + 0.009, mc.thePlayer.posZ, false))
                        mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(mc.thePlayer.posX, mc.thePlayer.posY +  0.009, mc.thePlayer.posZ, false))
                        mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(mc.thePlayer.posX, mc.thePlayer.posY + 0.009, mc.thePlayer.posZ, false))
                        mc.thePlayer.onCriticalHit(target)
