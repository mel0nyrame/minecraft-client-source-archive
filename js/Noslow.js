...
function isBlocking() {
    return mc.thePlayer.isBlocking() || KillAuraClass.blockingStatus
}
script.registerModule({
    name: "NoSlowDown",
    description: "NoSlow By ArrisDream",
    category: "Movement",
    tag: packetMode.get(),
    settings: {
    ...
    module.on("motion", function (event) {
        ...
        if (isBlocking() && (mc.thePlayer.motionX || mc.thePlayer.motionZ || MovementUtils.isMoving()) && heldItem && heldItem.getItem() instanceof ItemSword) {
            if (
                (packetMode.get() == "AAC" && mc.thePlayer.ticksExisted & 1) ||
