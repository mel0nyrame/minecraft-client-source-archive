...
class Aura : Module() {
    ...
    private val maxCPS: IntegerValue = object : IntegerValue(ObfuscatorUtils.obf("乭乁乘乣买乳"), 12, 1, 20) {
        override fun onChanged(oldValue: Int, newValue: Int) {
            ...
            when(cpsMode.get()) {
                ...
                "Light" -> {
                    attackDelay = TimeUtils.randomDelayOther(minCPS.get(), this.get())
                }
            }
        }
    }
    ...
    private val minCPS: IntegerValue = object : IntegerValue(ObfuscatorUtils.obf("乭义乎乣买乳"), 9, 1, 20) {
        override fun onChanged(oldValue: Int, newValue: Int) {
            ...
            when(cpsMode.get()) {
                ...
                "Light" -> {
                    attackDelay = TimeUtils.randomDelayOther(this.get(), maxCPS.get())
                }
            }
        }
    }
    ...
    private val cpsMode = ListValue("CPS", arrayOf("LiquidBounce", "Light"), "LiquidBounce")
    ...
    private val autoBlockPacketValue = ListValue("AutoBlockPacket", arrayOf("Normal","AfterTick", "AfterAttack", "Vanilla","HVH"),"AfterTick")
    ...
    private val rotationModeValue = ListValue("RotationMode", arrayOf("Novoline","Exhibition","Lock","Vanilla","NCP","BackTrack"), "Lock")
    ...
    private val rotationStrafeValue = ListValue("Strafe", arrayOf("Off", "Strict", "Silent","AAC4.4.0"), "Slient")
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        when(markValue.get().toLowerCase()){
            "liquid" -> {
                discoveredTargets.forEach {
                    RenderUtils.drawPlatform2(it, if (it.hurtTime<=0) Color(37, 126, 255, 170) else Color(255, 0, 0, 170))
                }
            }
            "block" -> {
                discoveredTargets.forEach {
                    ...
                    RenderUtils.drawEntityBox2(it, if (it.hurtTime<=0) Color.GREEN else Color.RED, true, true, 4f)
        ...
        if (currentTarget != null && attackTimer.hasTimePassed(attackDelay) &&
            ...
            when(cpsMode.get()) {
                ...
                "LightLight" -> {
                    attackDelay = TimeUtils.randomDelayOther(minCPS.get(), maxCPS.get())
                }
            }
        }
    }
    ...
    private fun attackEntity(entity: EntityLivingBase) {
        ...
        if(autoBlockPacketValue.get() == "Normal")
    ...
    private fun updateRotations(entity: Entity): Boolean {
        ...
        if (rotationModeValue.get().equals("Vanilla", ignoreCase = true)){
            if (maxTurnSpeed.get() <= 0F)
                return true
        ...
        if (rotationModeValue.get().equals("NCP", ignoreCase = true)){
            if (maxTurnSpeed.get() <= 0F)
                return true
        ...
        val limitedRotation = RotationUtils.limitAngleChange(RotationUtils.serverRotation, when (rotationModeValue.get()) {
            "Novoline" -> RotationUtils.getNovolineRotation(entity)
            "Exhibition" -> RotationUtils.getRotations2(entity)
    ...
    private fun startBlocking(interactEntity: Entity, interact: Boolean) {
        ...
        if(autoBlockPacketValue.get() == "hvh")
